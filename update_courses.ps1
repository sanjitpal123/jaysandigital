...
# Perform regex replace to extract everything between <main> and </main>
$pattern = "(?s)<main.*?</main>\s*(?=<!--\s*\d*\.?\s*Expanded Footer|<footer)"
$newContent = [regex]::Replace($content, $pattern, $newMain + "`n`n    ")

# In case the file has the encoding issue, replace those as well
$newContent = $newContent -replace 'Â·', '&middot;'
