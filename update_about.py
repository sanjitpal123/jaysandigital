...
def update_about_html():
    ...
    # Find where <main starts and </main> ends, or rather where <main starts and <footer starts.
    # We want to replace everything from <main ... > to <footer ... > 
    
    pattern = re.compile(r'<main.*?</main>\s*(?=<!--\s*\d*\.?\s*Expanded Footer\s*-->|<footer)', re.DOTALL | re.IGNORECASE)
    ...
    # We need to replace everything from <main...> to </main> AND any other sections before <footer...>
    # Actually, the file currently has sections outside of <main> too, because the user's index.html had multiple <section>s.
    
    # Wait, the current about.html has <main> ... </main> and then <section> ... </section> up to <footer ...>
    # So we should regex from <main to <footer
